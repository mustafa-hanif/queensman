...
CREATE TABLE public.callout (
    ...
    urgency_level character varying(255) DEFAULT NULL::character varying NOT NULL,
    ...
    CONSTRAINT callout_category_check CHECK (((category)::text = ANY ((ARRAY['AMC'::character varying, 'Ad-hoc'::character varying, 'Renovation'::character varying, 'Metered Service'::character varying, 'Uncategorized'::character varying])::text[]))),
    CONSTRAINT callout_inserted_by_check CHECK (((inserted_by)::text = ANY ((ARRAY['Admin'::character varying, 'Customer'::character varying, 'Ops Team'::character varying])::text[]))),
    CONSTRAINT callout_status_check CHECK (((status)::text = ANY ((ARRAY['Planned'::character varying, 'Requested'::character varying, 'Scheduled'::character varying, 'Closed'::character varying, 'Job Assigned'::character varying, 'In Progress'::character varying, 'Cancelled'::character varying])::text[]))),
    CONSTRAINT callout_urgency_level_check CHECK (((urgency_level)::text = ANY ((ARRAY['High'::character varying, 'Medium'::character varying, 'Scheduled'::character varying])::text[])))
);
...
CREATE TABLE public.job_history (
    ...
    updated_by character varying(50) DEFAULT NULL::character varying NOT NULL,
...
CREATE TABLE public.worker (
    ...
    phone character varying(14) DEFAULT NULL::character varying NOT NULL,
    description character varying(255) DEFAULT NULL::character varying,
    color_code character varying(6) DEFAULT NULL::character varying,
    team_id integer,
    active smallint DEFAULT 1 NOT NULL
);
...
ALTER TABLE ONLY public.job_worker
    ADD CONSTRAINT job_worker_pkey PRIMARY KEY (callout_id, worker_id);
...
ALTER TABLE ONLY public.scheduler
    ADD CONSTRAINT scheduler_callout_id_fkey FOREIGN KEY (callout_id) REFERENCES public.callout(id) ON UPDATE RESTRICT ON DELETE RESTRICT;
ALTER TABLE ONLY public.scheduler
