...
export default class ServicesHistory extends React.Component {
    ...
    async  componentWillMount() {
        ...
        link="https://www.queensman.com/phase_2/queens_worker_Apis/fetchServicesHistoryViaWorkerID.php?ID="+this.state.workerID
        console.log(link)
    ...
    render() {
        return (
            <View style={styles.container}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: '6%', marginBottom: '4%' }}>
                    <Text style={{ fontSize: 20, fontWeight: '500', color: '#FFCA5D', marginRight: '4%' }}>Sort</Text>
                                        ...
                                        <TouchableOpacity onPress={() => this.passItem(item)}>
                                            <View style={styles.Card}>
                                                ...
                                                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                                    <Text style={[styles.TextFam, { fontSize: 15, fontWeight: 'bold', }]}>Job type : {item.service_details.job_type}</Text>
                                                    <Icon name='flag' style={{ fontSize: 24, color:item.service_details.urgency_level=='High'?'red':item.service_details.urgency_level=='Scheduled'?"#aaa": '#FFCA5D', paddingRight: '5%' }}></Icon>
                                                </View>
                                                ...
                                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', flex: 1, paddingBottom: '5%', }}>
                                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                        ...
                                                        <View style={{ flexDirection: 'column' }}>
                                                            <Text style={[styles.TextFam, { fontSize: 10 }]}>Callout ID : {item.service_details.id}</Text>
                                                            <Text style={[styles.TextFam, { fontSize: 10 }]}>Property ID : {item.service_details.property_id}</Text>
                                                        </View>
                                                    </View>

                                                    <Text style={[styles.TextFam, { fontSize: 9, color: '#aaa', paddingRight: '5%', alignSelf: 'center' }]}>Resolved time : {item.service_details.resolved_time} </Text>
