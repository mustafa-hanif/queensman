...
export default class PreJob extends React.Component {
    ...
    componentWillMount() {
        link = "https://www.queensman.com/phase_2/queens_worker_Apis/fetchOngoingJobPrePictures.php?ID=" + this.state.CallOutID
        console.log(link)
        axios.get(link).then((result) => {
            console.log(result.data.server_response[0].ongoing_job_pictures.picture_location)
            if (result.data.server_response != -1) {
                if (result.data.server_response[0].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic1: result.data.server_response[0].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[1].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic2: result.data.server_response[1].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[2].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic3: result.data.server_response[2].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[3].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic4: result.data.server_response[3].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[4].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic5: result.data.server_response[4].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[5].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic6: result.data.server_response[5].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[6].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic7: result.data.server_response[6].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[7].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic8: result.data.server_response[7].ongoing_job_pictures.picture_location,
                    })
                }
                if (result.data.server_response[8].ongoing_job_pictures.picture_location) {
                    this.setState({
                        IsImageuploaded: true,
                        Pic9: result.data.server_response[8].ongoing_job_pictures.picture_location,
                    })
                }
            }
    ...
    FetchUpdateNotes() {
        link = "https://www.queensman.com/phase_2/queens_worker_Apis/fetchJobNotes.php?ID=" + this.state.CallOutID
        console.log(link)
        axios.get(link).then((result) => {
    ...
    UploadImage = () => {
        if (this.state.Pic1 != 'link') {
            link = "https://www.queensman.com/phase_2/queens_worker_Apis/uploadPrePicture_a.php"
            if (this.state.Pic1 != "link") {
    ...
    urlToUPLOAD = async (url, link) => {
        ...
        blink = "https://www.queensman.com/phase_2/queens_worker_Apis/uploadPrePicture_b.php?target_file=" + filename + "&ID=" + this.state.CallOutID
        console.log(blink)
        axios.get(blink).then((result) => {
            console.log(result.data)
        })
    ...
    addNote = () => {
        ...
        link = "https://www.queensman.com/phase_2/queens_worker_Apis/insertJobNotes.php?ID=" + this.state.CallOutID + "&note=" + this.state.Note
        console.log(link)
        axios.get(link).then((result) => {
            console.log(result.data)
            this.setState({
    ...
    RemoveNotes = (item) => {
        link = "https://www.queensman.com/phase_2/queens_worker_Apis/deleteJobNotes.php?ID=" + this.state.CallOutID + "&note=" + item.note
        console.log(link)
        axios.get(link).then((result) => {
            console.log(result.data)
        ...
        return (
            <KeyboardAvoidingView style={{ flex: 1 }} behavior="padding" enabled>
                <ScrollView style={styles.container}>
                    <Text style={{ fontSize: 15, fontWeight: '500', color: '#FFCA5D', marginBottom: '1.5%' }}>Pre Images</Text>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',paddingHorizontal:'5%', opacity: (this.state.IsImageuploaded ? 0.3 : 1) }} pointerEvents={this.state.IsImageuploaded ? 'none' : 'auto'}>
                        ...
                        <TouchableOpacity onPress={this.SelectImages} disabled={this.state.UploadButtonDeactive}>

                            <View style={{ flexDirection: 'row', width: '100%', alignItems: 'center', }}>

                                <Icon name="add-circle" style={{ fontSize: 20, color: '#000E1E', paddingRight: "3%", }}></Icon>
                                <Text style={{ fontSize: 13, marginBottom: '1%' }}>Select Image From Gallery</Text>
                    ...
                    <Button
                        ...
                        color="#FFCA5D"
                    />
                    <Text style={{ fontSize: 15, fontWeight: '500', color: '#FFCA5D', marginBottom: '1.5%', marginTop: '4%' }}>Notes: </Text>
                    <View style={{ heigh: '30%', width: '100%' }}>
                        <FlatList
                            data={this.state.Notes}
                            renderItem={({ item }) =>
                                <View style={{ flexDirection: 'row', paddingLeft: '4%', alignItems: 'center', justifyContent: 'space-between', paddingVertical: '1%' }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <Icon name='remove' style={{ fontSize: 12, color: '#aaa', }}></Icon>
                                        <Text style={{ fontSize: 12, color: '#aaa', }}> {item.note}</Text>
                                    </View>

                                    <TouchableOpacity onPress={() => this.RemoveNotesAlert(item)}>
                                        <Icon name='close-circle' style={{ fontSize: 20, color: '#FFCA5D', }}></Icon>
                                    </TouchableOpacity>
                                </View>
                            }
                        ></FlatList>
                    ...
                    <Text style={{ color: '#aaa', fontSize: 10 }}>Note: Kindly scroll screen upwards if the textbox hides behind the keyboard.</Text>
                    <View style={{ height: 2 }}></View>
                    <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                        <Icon name="paper" style={{ fontSize: 25, color: '#000E1E', paddingRight: "4%" }}></Icon>
                        <TextInput
                            ref="textInputMobile"
                            ...
                            onChangeText={(Note) => { this.setState({ Note }); }}//email set
                        />
                        <TouchableOpacity onPress={this.addNote}>
                            <Icon name="add-circle" style={{ fontSize: 25, color: '#000E1E', }}></Icon>
                        </TouchableOpacity>
                    </View>
