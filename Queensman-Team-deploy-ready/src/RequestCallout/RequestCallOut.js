...
export default class RequestCallOut extends React.Component {
    ...
    async componentWillMount() {
        ...
        link = "https://www.queensman.com/phase_2/queens_client_Apis/FetchClientSinglePropertyViaPropID.php?ID=" + this.props.navigation.getParam('it').property_id
        console.log(link);
        ...
        if (this.state.picture1 == '' || this.state.Urgency == "" || this.state.JobType == 'none') {
            ...
            NetInfo.isConnected.fetch().then(isConnected => {
                if (isConnected) {
                    ...
                    link = "https://www.queensman.com/phase_2/queens_worker_Apis/submitCallOutFromTeam.php?client_id=" + this.props.navigation.getParam('clientID') + "&prop_id=" + this.props.navigation.getParam('it').property_id + "&job=" + JOBS + "&describe=" + this.state.Description + "&property_type=" + this.props.navigation.getParam('it').property_category + "&urg_lvl=" + this.state.Urgency + "&picture1=" + pic1name + "&picture2=" + pic2name + "&picture3=" + pic3name + "&picture4=" + pic4name + "&ops_team_id=" + this.state.workerID
                    console.log(link);
                    axios.get(link).then(result => {
                        console.log(result.data.server_responce)

                        link = "https://www.queensman.com/phase_2/queens_client_Apis/uploadPhoto.php"
                        if (this.state.picture1 != '') {
                        ...
                        setTimeout(() => {
                            link = "https://www.queensman.com/phase_2/queens_client_Apis/fetchNewCalloutID.php?ID=" + this.props.navigation.getParam('clientID')
                            console.log(link);
                            axios.get(link).then((result) => {
                                ...
                                setTimeout(() => {
                                    ...
                                    link = "https://www.queensman.com/phase_2/queens_client_Apis/fetchClientProfile.php?ID=" + this.props.navigation.getParam('clientID')
                                    console.log(link);
                                    ...
                                    axios.get(link).then((result) => {
                                        ...
                                        link = "https://www.queensman.com/phase_2/queens_worker_Apis/sendEmailForRequestCallout.php?subject=" + this.state.subject + "&callout_id=" + this.state.CalloutID + "&client_id=" + this.props.navigation.getParam('clientID') + "&client_name=" + this.state.UserName + "&client_email=" + this.state.Email + "&job=" + JOBS + "&description=" + this.state.Description + "&callout_urgency=" + this.state.Urgency + "&property_id=" + this.props.navigation.getParam('it').property_id + "&property_address=" + this.state.address + "&community=" + this.state.community + "&city=" + this.state.city + "&country=" + this.state.country + "&client_phone=" + this.state.Mobile
                                        console.log(link);
                                        axios.get(link).then(result => {
