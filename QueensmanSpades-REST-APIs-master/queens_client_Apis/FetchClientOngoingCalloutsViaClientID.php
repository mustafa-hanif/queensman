...
$mysql_qry = "SELECT callout.id, callout.property_id, callout.job_type, callout.description, callout.status, DATE_FORMAT (callout.request_time, '%b %d, %Y') AS request_time ,
                     DATE_FORMAT (callout.resolved_time, '%b %d, %Y') AS resolved_time, callout.picture1, callout.picture2, callout.picture3, callout.picture4 , callout.urgency_level, client.id as client_id, client.email as client_username, property.id as property_id, property.address, property.community, property.city, property.country
              FROM callout
              INNER JOIN client ON callout.callout_by = client.id
INNER JOIN property ON callout.property_id = property.id
              WHERE callout.active = 1 AND callout.callout_by = '$user_ID' AND (callout.status != 'Closed' AND callout.status != 'Cancelled')
