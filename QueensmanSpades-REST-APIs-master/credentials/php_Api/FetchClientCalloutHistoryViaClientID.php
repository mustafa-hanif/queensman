...
$mysql_qry = "SELECT callout.id, callout.property_id, callout.job_type, callout.description, callout.status, 
		...
		callout.picture1, callout.picture2, callout.picture3, callout.picture4 , 
		callout.urgency_level, job.rating, job.feedback, property.address,property.community,
		property.city,property.country, client.id as client_id, client.email as client_username 
	     FROM callout 
	     LEFT JOIN job ON callout.id = job.id INNER JOIN client ON callout.callout_by = client.id 
	     INNER JOIN property ON property.id = callout.property_id 
	     WHERE callout.callout_by = '$user_ID' AND (callout.status = 'Resolved' OR callout.status = 'Cancelled');";
