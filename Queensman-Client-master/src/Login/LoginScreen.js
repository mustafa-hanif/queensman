...
import { StyleSheet, Text, View, Dimensions, Image, TextInput, ImageBackground, NetInfo, TouchableOpacity, Linking, ActivityIndicator, KeyboardAvoidingView, AsyncStorage } from 'react-native';
...
export default class LoginScreen extends React.Component {
    ...
    scanFingerprint = async () => {
        ...
        if (result.success) {
            link = "http://13.250.20.151/queens_client_Apis/fetchDeviceID.php?email=" + this.state.email
            console.log(link);
        ...
        if (this.state.connections) {
            ...
            axios.get(link).then((result) => {
                ...
                if (result.data.server_responce == -1) {
                    alert('Either you are not a registered user or the email you entered is incorrect. Kindly contact us for more details.');
                    this.setState({ loading: false })
                }
                else {
                    link = "http://13.250.20.151/queens_client_Apis/fetchClientID.php?email=" + this.state.email
                    console.log(link);
                    axios.get(link).then((result) => {
    ...
    _storeData = async (id) => {
        ...
        try {
            ...
            if (this.state.passwordcheck == "null") {
                ...
                var active = "";
                link = "http://13.250.20.151/queens_client_Apis/fetchClientActive.php?email=" + this.state.email
                console.log(link);
                axios.get(link).then((result) => {
                    console.log(result.data.server_responce_ID);
    ...
    //animation function
    fadeOutLeft = () => this.view.fadeOutLeftBig(400).then(endState => console.log(endState.finished ? 'bounce finished' : 'bounce cancelled'));
    fadeOutRight = () => this.view.fadeOutRightBig(400).then(endState => console.log(endState.finished ? 'bounce finished' : 'bounce cancelled'));
            ...
            <KeyboardAvoidingView style={styles.container} behavior="padding" enabled>
                ...
                {this.state.emailpage ?
                    <Animatable.View style={{ width: '100%', }} ref={this.handleViewRef}   >
                        ...
                        <ImageBackground
                            style={{ height: 43, width: 280, alignSelf: 'center', justifyContent: 'center' }}
                        ...
                        <TouchableOpacity style={{ height: 38, width: 200, alignSelf: 'center' }} onPress={this.proceedFunctionEmail}>
                            <ImageBackground
                    ...
                    <Animatable.View style={{ width: '100%', alignItems: 'center' }} animation="fadeInRightBig" iterationCount={1} ref={this.handleViewRefs} >

                        <ImageBackground
                            style={{ width: 280, height: 43 }}
                            source={require('../../assets/Login/Password_field4.png')}>
                            <View style={{ justifyContent: 'center', paddingTop: '2%', paddingLeft: '15%', flexDirection: 'row' }}>
                                <TextInput

                                    ref="textInputMobile"
                                    style={{ fontSize: 15, fontFamily: 'Helvetica', width: '80%' }}
                                    placeholder='Password'
                                    autoCapitalize='none'
                                    secureTextEntry={this.state.showPassword}
                                    underlineColorAndroid="transparent"
                                    onChangeText={(password) => { this.setState({ password }); }}//password set
                                />
                                {this.state.showPassword ? <Icon onPress={this.toggleSwitch} name="eye-off" style={{ paddingTop: '2%', fontSize: 25, color: '#000E1E', paddingRight: "4%", }}></Icon> : <Icon onPress={this.toggleSwitch} name="eye" style={{ paddingTop: '2%', fontSize: 25, color: '#000E1E', paddingRight: "4%", }}></Icon>}
                            </View>
                        </ImageBackground>
                        ...
                        <Text style={{ fontSize: 10, fontFamily: 'Helvetica', color: '#fff', alignSelf: 'center', paddingTop: '3%' }}>You may also use your phone fingerprint to login</Text>
                        {/* go back to email */}
                ...
                {this.state.loading ? <ActivityIndicator size='large' color="#fff" style={{ paddingTop: '5%', }} /> : <View></View>}


                {/* call us */}

                <View style={{ borderBottomColor: '#FFCA5D', borderBottomWidth: 1, width: 280, paddingTop: '17%', alignSelf: 'center' }}></View>
                {this.state.emailpage ?
                    <TouchableOpacity onPress={this.ContectUsFuntion}>
                        <View style={{ flexDirection: 'column', paddingTop: '5%' }}>
                            ...
                            <Text style={{ fontSize: 20, fontFamily: 'Helvetica', color: '#FFCA5D', textDecorationLine: 'underline' }}>CONTACT US</Text>
                    ...
                    : <TouchableOpacity onPress={this.CallUsFunction}>
                        <View style={{ flexDirection: 'column', paddingTop: '5%' }}>
                            ...
                            <Text style={{ fontSize: 11, fontFamily: 'Helvetica', color: '#fff', alignSelf: 'center' }}>Facing any issues?</Text>
                            <View style={{ flexDirection: 'row' }}>
                                <Image
                                    style={{ height: 18, width: 18, marginTop: '3%' }}
                                    source={require('../../assets/Login/Phone.png')} />
                                <Text>   </Text>
                                <Text style={{ fontSize: 20, fontFamily: 'Helvetica', color: '#FFCA5D' }}>CALL US NOW</Text>
                            </View>
                        </View>
                    </TouchableOpacity>}
                ...
                <TouchableOpacity onPress={this.contactEmailHandler}>
                    <View style={{ flexDirection: 'row', alignSelf: 'center', alignItems: 'center' }}>
                        ...
                        <Text style={{ fontSize: 11, fontFamily: 'Helvetica', color: '#FFCA5D', }}>services@queensman.com</Text>
                    </View>
                </TouchableOpacity>
...
const styles = StyleSheet.create({
    ...
    LogoStyle: {
        height: 230, width: 230, marginBottom: "10%",
